...
<h2><center>สร้างสูตรส่วนผสมขนม<center> </h2>
            ...
            <table id="example" class="display nowrap" style="width:100%">
                    <thead>
                        <tr>
                            <th>ชื่อส่วนผสม</th>
							<th>น้ำหนักที่ต้องใช้ต่อชิ้น</th>
                        </tr>
                    </thead>
                    <tbody>
                    
                        @foreach($dataset['ingname'] as $var)

                    <tr> 
                        <td>
                        <input type="checkbox" value="{{$var->ing_id}}">{{$var->ing_name}} ({{$var->ing_id}})</input>
                        </td>
							<td><input type="number" id="weighttxt" name="weight[]" class="form-control"></td>
                        </tr>
                        @endforeach
...
function SaveFormula(){
	selectgroupdata = $("#selectgroupdata").val();
 checkForid = [];
//  console.log(checkArr);
 ...
 $('input[name="weight[]"]').each(function(){
	...
	weight.push(str);
	 ...
	 $.ajax({
	type:"POST",
	url:'{{URL::to("CreateFormula/saveFormula")}}', 
	data:{
		selectgroupdata: selectgroupdata,
		checkForid: checkForid,
		weight: weight,
	   _token:"{{ csrf_token() }}"
