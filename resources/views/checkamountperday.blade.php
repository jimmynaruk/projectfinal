...
<h2><center>รายการที่ต้องผลิตวันนี้<center> </h2>
            <div class="container-fluid">
            ...
            <table id="example" class="display nowrap" style="width:100%">
                    ...
                    <tbody>
                        ...
                        @foreach($dataset['amountdesday'] as $index => $var)

                    <tr> 
						
					<td><input type="text" id="proid" name="proid[]"  value="{{$var->product_id}} " class="form-control"readonly></td>
						<td> {{$var->product_name}}</td>
                       
						<td><input type="text" id="caltxt" name="caltxt[]"  value="{{$var->total_amount}}" class="form-control"readonly> </td>
                ...
                <button type="button"  class="btn btn-primary" onclick="linkResetCapacity()"  >
                <span class="fa fa-save" aria-hidden="true"></span>&nbsp;ดำเนินการผลิตขั้นต่อไป</button>
  </div>
  </div>
  </div>
...
<script>
function linkResetCapacity(){

	window.location.href = "{{URL::TO('manageProductivity')}}"
...
caltxt = [];
         $('input[name="caltxt[]"]').each(function(){
			if(this.value){
            var str = this.value;
            caltxt.push(str);
			}
...
$.ajax({
type:"POST",
url:'{{URL::to("checkamountperday/uptotal")}}',
data:{
	proid: proid,
	caltxt: caltxt,
_token:"{{ csrf_token() }}"
		...
		window.location.href = "http://localhost/projectfinal/public/CalIngredient"
		...
		console.log(status);
		console.log(error);
		alert("ไม่มีขนมที่ต้องผลิตตอนนี้ หรือ ยังไม่ได้สร้างส่วนผสมกรุณาตรวจสอบ")
	}
})
