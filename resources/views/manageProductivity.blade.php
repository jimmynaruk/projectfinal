...
<html lang="en">
<head>
	<title>รายการวัตถุดิบส่วนผสม</title>
	<meta charset="UTF-8">
...
<h2><center>จัดการกำลังผลิตของแต่ละวัน<center> </h2>
<div class="container">
<div class="card shadow mb-4">
<div class="card-body">

<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal">
<span class="fa fa-plus" aria-hidden="true"></span>&nbsp;Reset</button><p>
...
<table id="example" class="display" style="width:100%">
        ...
        <tr>
            <th>รหัสวัตถุดิบ</th>
            <th>ชื่อวัตถุดิบ</th>
			<th>จำนวนวัตถุดิบในคลัง/กิโลกรัม</th>
            <th>จำนวนวัตถุดิบต่อวัน/กิโลกรัม</th>
            <th>จำนวนที่ซื้อเพิ่มเข้ามา</th>
        </tr>
        </thead>
        <tbody>
        
		@foreach($dataset['dataNewingredient'] as $var)
           <tr>
                <td><input type="text"id="ingidname" name="ingidname[]"value="{{$var->ing_id}}"  class="form-control" readonly></td>
                <td>{{$var->ing_name}}</td>
				<td>{{$var->capacity}}</td>
                <td><input type="text"id="newcapacity" name="newcapacity[]"value="{{$var->sum_quantity}}"  class="form-control" readonly></td>
                <td>{{abs($var->added)}}</td>
            </tr>
			@endforeach
        </tbody>
...
<div class="modal fade" id="exampleModal" >
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h3 class="modal-title" id="exampleModalLabel">อัปเดตกำลังผลิต</h3>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
         ...
         <form>
          ...
          <div class="form-group">
            ...
            <input type="number" class="form-control" value="3000"id="txtperday" readonly>
          </div>
          </div>
      ...
      <div class="modal-footer">
        ...
        <button type="button"  onclick="SaveFormula();"class="btn btn-primary">
...
/*function linkCal(){
	window.location.href = "{{URL::TO('CalIngredient')}}"
}*/
  function UpdateStock(){
	...
	ingidname = [];
         $('input[name="ingidname[]"]').each(function(){
		  ...
		  newcapacity = [];
         $('input[name="newcapacity[]"]').each(function(){
			if(this.value){
            var str = this.value;
            newcapacity.push(str);
			}
          });
		 
$.ajax({
type:"POST",
url:'{{URL::to("manageProductivity/UpdateCapacity")}}',
data:{
	ingidname: ingidname,
...
$.ajax({
type:"POST",
url:'{{URL::to("manageProductivity/EditProductivity")}}',
data:{
	txtproid: txtproid,
	txtperday: txtperday,
_token:"{{ csrf_token() }}"
