...
<table id="example" class="display nowrap" style="width:100%">
		...
		@foreach($dataset['dataorder'] as $var)
			<tr>
				...
				<td>{{$var->status}}</td> 
                <td> 
				<button type="button"  class="btn btn-success" onclick="findpic('{{$var->order_id}}')" >
               <span class="fa fa-eye"  aria-hidden="true"></span>&nbsp;ตรวจหลักฐานการชำระเงินและรับรายการ</button>
			   <button type="button"  class="btn btn-danger" onclick="reject('{{$var->order_id}}')" >
               <span class="fa fa-ban"  aria-hidden="true"></span>&nbsp;ยกเลิกรายการ</button>
...
<div class="modal fade" id="eyemodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
		...
		<div class="modal-content">
			...
			<h5 class="modal-title" id="exampleModalLabel">ดูใบเสร็จการโอนเงินชำระค่าขนม</h5>
			...
			<button type="button" onclick="updatestatus()" class="btn btn-success">รับรายการ</button>
			</div>
		</div>
		</div>
	</div>
    ...
    <div class="modal-dialog" role="document">
      ...
      <textarea class="form-control  " rows="5" id="comment"></textarea>
        ...
        <div class="modal-footer">
          ...
          <button type="button"  onclick="RejectOrder() " class="btn btn-danger">ยกเลิก</button>
        </div>
      </div>
    </div>
  </div>
...
function updatestatus(){
	order_id = $("#orderidpic").val();
        ...
        $.ajax({
        type:"POST",
        url:'{{URL::to("checkpayment/acceptpayment")}}',
        data:{
          order_id: order_id,
          _token:"{{ csrf_token() }}"
...
function findpic(order_id){
   ...
   $.ajax({
    type:"POST",
    url:'{{URL::to("checkpayment/findpic")}}',
      data:{
		order_id: order_id,
        _token:"{{ csrf_token() }}"
      },success:function(result){
...
function reject(order_id){
  ...
  $.ajax({
	type:"POST",
	url:'{{URL::to("checkpayment/rejectOrder")}}',
	data:{
		order_id: order_id,
		comment: comment,
	  _token:"{{ csrf_token() }}"
