...
<h2><center>จัดการสิทธิ์<center> </h2>
            ...
            @foreach($dataset['mapgroup'] as $data)
            <option value="{{$data->group_code}}">{{$data->group_name}}</option>
            @endforeach
            </select>
            </div>
            </div><br>
            ...
            <table id="example" class="display nowrap" style="width:100%">
                    ...
                    <tbody>
                        ...
                        @foreach($dataset['func'] as $var)

                    <tr> 
                        <td>
                        <input type="checkbox" value="{{$var->function_id}}">
                        </td>
                ...
                <button type="button"  class="btn btn-primary" onclick="saveMp()"  >
...
// 	}
  ...
  function saveMp(){
    group_code = $('#selectgroupdata').val();
    mpArr = [];
    console.log(saveMp);
     $('input[type="checkbox"]').each(function(){
      ...
      $.ajax({
        type:"POST",
        url:'{{URL::to("manageprivilege/saveMP")}}', 
        data:{
          mpArr : mpArr,
